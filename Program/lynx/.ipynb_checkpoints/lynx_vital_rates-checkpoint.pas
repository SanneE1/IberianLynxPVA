...
uses
  ...
  lynx_dispersal_assist_functions, general_functions, general_define_units;
...
procedure Reproduction;
var
  a, x,y, current_litter_size, ls, xy, CurrentDist: integer;
              ...
              begin
                ...
                begin
                   ...
                   begin
                     ...
                     for x := Lynx^.Coor_X - CurrentDist to Lynx^.Coor_X + CurrentDist do
                     begin
                     for y := Lynx^.Coor_Y - CurrentDist to Lynx^.Coor_Y + CurrentDist do
                     begin

                     // only checks cells on the "ring" at CurrentDist
                     if (Max(Abs(x- Lynx^.Coor_X), Abs(y - Lynx^.Coor_Y)) <> CurrentDist) then
                     Continue;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Lynx^.Status = 1) then surv_p := L_surv_disperse
      else
        begin
          ...
          if (Lynx^.Status > 1) and (Lynx^.Age <= L_max_rep_age) then
            surv_p := L_surv_resident
            else
          if (Lynx^.Age > L_max_rep_age) then surv_p := L_surv_old;
        end;
      ...
      begin
       daily_mortality_p := (1-surv_day) + ((1-surv_day) * L_surv_disp_rho * (Lynx^.DailyStepsOpen / Lynx^.DailySteps));
       surv_day := 1-daily_mortality_p;
      end;

      {Determine fate of Lynxs}
      die := False;
      if Lynx^.age > L_max_age then die := True
      else
      ...
      begin
       ...
       for b := 0 to length(Lynx^.TerritoryX) - 1 do
          begin
            ...
            if Lynx^.Sex = 'f' then
            begin
            FemalesMap[Lynx^.TerritoryX[b], Lynx^.TerritoryY[b], 0]:= 0;
            FemalesMap[Lynx^.TerritoryX[b], Lynx^.TerritoryY[b], 1]:= 0;
            end
            else
            begin
            MalesMap[Lynx^.TerritoryX[b], Lynx^.TerritoryY[b], 0]:= 0;
            MalesMap[Lynx^.TerritoryX[b], Lynx^.TerritoryY[b], 1]:= 0;
            end;
            Lynx^.TerritoryX[b] := -1;
            Lynx^.TerritoryY[b] := -1;
          end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (TCount < Tsize) and (TCount > 0) then //If TCount is 0 that means that the Lynx will have to move for sure to find new territory
      ...
      begin
        ...
        begin
          ...
          if (HabitatMapLynx[TestCoordX, TestCoordY] = 2) and (ReproductionQuality(TestCoordX, TestCoordY)) then
          begin
            ...
            begin
              ...
              begin
               ...
               temp_ind := FindTerrOwner(LynxPopulation, 'f', TestCoordX, TestCoordY);
