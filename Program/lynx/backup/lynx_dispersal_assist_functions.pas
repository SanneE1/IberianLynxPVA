...
begin
  Result:=False;
  if (BreedingHabitatMap[x,y] = 2) and (whichPop(x,y) > 0) then Result := True;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (p < theta) and CanMoveHere((xp + dx[mem]), (yp + dy[mem])) then Result := mem
      else
        if f < (theta + theta * L_theta_delta) then
        begin
          if (mem > 4) and CanMoveHere((xp + dx[mem-4]), (yp + dy[mem-4])) then Result := mem - 4
          else
            if (mem > 0) and (mem <= 4) and CanMoveHere((xp + dx[mem+4]), (yp + dy[mem+4])) then Result := mem + 4;
        end;
      if Result > 10 then  // If new direction has not yet been assigned through autocorrelation, random movement
      begin
   ...
   use autocorrelation to see if ind. moves in memory direction.}
        ...
        {probability of moving backwards to autocorrelation}
          if (p < (theta + theta * L_theta_delta)) then
          begin
            if (mem = 0) and CanMoveHere((xp + dx[mem]), (yp + dy[mem])) and (HabitatMapLynx[(xp + dx[mem]), (yp + dy[mem])] = 2) then
              Result := mem
            else
              if (mem > 4) and CanMoveHere((xp + dx[mem-4]), (yp + dy[mem-4])) and (HabitatMapLynx[(xp + dx[mem - 4]), (yp + dy[mem - 4])] = 2) then
                Result := mem - 4
              else
                if (mem > 0) and (mem <= 4) and CanMoveHere((xp + dx[mem+4]), (yp + dy[mem+4])) and (HabitatMapLynx[(xp + dx[mem + 4]), (yp + dy[mem + 4])] = 2) then
                  Result := mem + 4;
          end;
        {Otherwise a random choise of Dispersal habitat cells}
        if (Result > 10) and (nDisp > 0) then
        begin
          h := random(nDisp) + 1;
          f := 0;
  ...
  if Result = 99 then
    Exit;
  if HabitatMapLynx[(xp + dx[Result]), (yp + dy[Result])] < 1 then
    Exit;
  if CanMoveHere((xp + dx[Result]), (yp + dy[Result])) = False then
    Exit;
