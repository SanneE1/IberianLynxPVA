...
plot_emigration <- function(emigration_file, save_graph = F, save_path = NULL){
  library(terra)
  library(sf)
  library(dplyr)
  library(ggplot2)
  ...
  events <- read.csv(emigration_file)
  colnames(events) <- colnames(events)[-2]
  events[,8] <- NULL
  colnames(events)[8] <- "source_run"
  ...
  if (save_graph) {
    ggsave(plot, filename = save_path, width = 8, height = 8)
  }
