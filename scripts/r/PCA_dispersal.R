...
# --- Output directory ---------------------------------------------------------
OUT_DIR <- "data/GIS_maps/PCA_dispersal"   
...
r_dispersal  <- terra::rast(dispersal_path)  # Dispersal resistance was multiplied by 10000
# r_dispersal  <- r_dispersal / 10000          # returning it to the 0-1 scale
...
# CRS MATCHING ----------------------------------------------------------------
r_dispersal <- terra::project(r_dispersal, terra::crs(r_template), method = "bilinear")
...
# EXTENT CROPPING -------------------------------------------------------------
r_dispersal <- terra::crop(r_dispersal, r_template)
r_elev <- terra::crop(r_elev, r_template)
r_veg  <- terra::crop(r_veg,  r_template)
r_road    <- terra::crop(r_road, r_template)

# RESCALE ---------------------------------------------------------------------
r_dispersal <- terra::resample(r_dispersal, r_template, method = "bilinear")
...
names(r_dispersal)  <- "dispersal.resistance"
...
full_stack <- c(r_dispersal, r_elev, r_veg, r_road)
...
gam_formula <- as.formula(sprintf(
  "log(dispersal.resistance) ~ s(elevation, k = %d, bs = 'cr') + s(road_density_presence_3x3, k = %d, bs = 'cr') + %s + s(x, y, k = %d, bs = 'tp') ",
...
hist <- predict_PCA_maps(veg_rast = hist_r, elev_rast = dem, road_rast = r_road,
                         ...
                         name_prediction_type = "predicted_dispersal",
...
writeRaster(fut_245_stack_pred, filename = file.path(OUT_DIR, "SSP245_PCA_prediction_dispersal.tif"))
writeRaster(fut_245_stack_pred, filename = file.path(OUT_DIR, "SSP245_PCA_prediction_dispersal.tif"))
...
writeRaster(fut_585_stack_pred, filename = file.path(OUT_DIR, "SSP585_PCA_prediction_dispersal.tif"), overwrite=T)
writeRaster(fut_585_stack_risk, filename = file.path(OUT_DIR, "SSP585_PCA_prediction_dispersal_exprisk.tif"), overwrite=T)
...
hist_predicted <- rast(file.path(OUT_DIR, "dispersal_historic.tif"))
if(!exists("r_dispersal")){
  r_dispersal  <- terra::rast(dispersal_path)  # Dispersal resistance was multiplied by 10000
  r_dispersal <- terra::project(r_dispersal, terra::crs(r_template), method = "bilinear")
  r_dispersal <- terra::crop(r_dispersal, r_template)
  r_dispersal <- terra::resample(r_dispersal, r_template, method = "bilinear")
...
df <- data.frame(
  disp_res = values(r_dispersal),
  disp_pred = values(hist_predicted),
...
df$corine <- factor(df$corine, ordered = TRUE)

tree <- rpart(corine ~ disp_res, data = df, method = "class",
...
for(t in seq_along(time(pred_files_585))){
  ...
  r <- ifel(r < splits[1], 2, ifel(r < splits[2], 1, 0))
  ...
  finput <- gsub("GIS_maps", "model_input/maps")
