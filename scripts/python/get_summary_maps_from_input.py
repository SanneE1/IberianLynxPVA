...
def calculate_matrix_statistics(folder_paths, output_folder, confidence_level=0.95):
    """
    Calculate mean and 95% CI for each cell across multiple txt matrices.
    ...
    # Get list of txt files from the first folder (assuming all folders have same files)
    first_folder = Path(folder_paths[0])
    txt_files = sorted(first_folder.glob("*.txt"))
    
    if not txt_files:
        print(f"No txt files found in {first_folder}")
        return
    ...
    for txt_file in txt_files:
        ...
        for folder in folder_paths:
            ...
            if file_path.exists():
                df = pd.read_csv(file_path, header=None, skiprows=1, delimiter=' ')
...
if __name__ == "__main__":
    ...
    folders = [str(f) for f in parent_dir.rglob('*') if f.is_dir() and f.parent != parent_dir]   
